...
async function checkStravaActivities(client) {
    ...
    try {
        ...
        /*if (!lastActivityId) {
            ...
            console.log(`Strava initialisé. Dernière activité : ${latest.name}`);
            return;
        }*/
        ...
        if (latest.id !== lastActivityId) {
            ...
            const pace = (latest.moving_time / 60 / (latest.distance / 1000)).toFixed(2);

            const embed = createBaseEmbed({ 
                username: `${latest.athlete.firstname} ${latest.athlete.lastname}`, 
                displayAvatarURL: () => 'https://images.icon-icons.com/2429/PNG/512/strava_logo_icon_147232.png'
            })
            .setColor('#fc4c02')
            .setTitle(`🏃 ${latest.name}`)
            .setDescription(`Une nouvelle activité a été postée dans le club !`)
            .addFields(
